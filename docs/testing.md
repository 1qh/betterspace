...
`tables` and `reducers` are auto-generated by `spacetime generate` from your SpacetimeDB
module. They provide typed accessors for your database tables and server functions.
...
`DbConnection` is the generated connection type from SpacetimeDB’s client SDK.
