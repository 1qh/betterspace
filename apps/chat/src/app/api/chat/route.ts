...
const WEATHER_LOCATION_RE = /weather(?:\s+in)?\s+([a-zA-Z\s-]+)/u,
  TRAILING_PUNCT_RE = /[?.!,]+$/u,
  ...
  getLastUserText = (messages: UIMessage[]): string => {
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      ...
      if (message && message.role === 'user') return getTextFromMessage(message)
    }
    return ''
  },
  getLatestApprovalPart = (messages: UIMessage[]): ToolPart | null => {
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      const message = messages[i]
      if (message) {
