...
const FUNCTION_NAME = Symbol.for('functionName'),
  ...
  api = makeApiProxy() as typeof BeApi,
  identityCache = new Map<string, IdentityResponse>(),
  identityKeyByUserId = new Map<string, string>(),
  ...
  toSnake = (s: string): string => s.replaceAll(/[A-Z]/gu, m => `_${m.toLowerCase()}`),
  // eslint-disable-next-line prefer-named-capture-group
  toCamel = (s: string): string => s.replaceAll(/_([a-z])/gu, (_m, c: string) => c.toUpperCase()),
  ...
  getHeaderValue = (): string => process.env.SPACETIMEDB_TEST_MODE ?? 'true'
...
const getReducerParams = async (): Promise<Map<string, SchemaElement[]>> => {
    ...
    reducerParamsPromise = (async () => {
      const response = await fetch(`${getHost()}/v1/database/${getModule()}/schema?version=9`),
        ...
        map = new Map<string, SchemaElement[]>(),
  ...
  extractSumType = (type: unknown): null | { variants: unknown[] } => {
  ...
  encodeArg = (type: unknown, value: unknown): unknown => {
    ...
    if (isRecord(type)) {
      ...
      for (const key of numericKeys) if (key in type && typeof value === 'string') return Number.parseInt(value, 10)
    }
    return value
  },
  ...
  decodeValue = (type: unknown, value: unknown): unknown => {
    ...
    if ('Sum' in type) {
      ...
      if (Array.isArray(value) && value.length === 2 && typeof value[0] === 'number') {
        ...
        const rawName = variant.name,
          variantType = variant.algebraic_type
    ...
    if ('Product' in type) {
      ...
      if (elementsRaw.length === 1) {
        ...
        const rawName = first.name,
          innerType = first.algebraic_type,
          [firstValue] = value
        if (isRecord(rawName)) {
          const name = rawName.some
      ...
      for (let i = 0; i < elementsRaw.length; i += 1) {
        ...
        if (isRecord(element)) {
          ...
          if (isRecord(rawName)) {
            ...
            if (name) obj[toCamel(name)] = decodeValue(element.algebraic_type, value[i])
          }
        }
      }
      return obj
  ...
  querySQL = async <T>(sql: string): Promise<T[]> => {
    const response = await fetch(`${getHost()}/v1/database/${getModule()}/sql`, {
      ...
      result = await parseJson<SqlResultSet[]>(response),
      [first] = result
    if (!first) return []
    ...
    for (const row of rows) {
      ...
      for (let i = 0; i < elements.length; i += 1) {
        ...
        if (name) entry[toCamel(name)] = decodeValue(elements[i]?.algebraic_type, row[i])
      }
      mapped.push(entry as T)
    }
    return mapped
  },
  toId = (value: unknown): string => String(value ?? ''),
  normalizeIdentity = (value: string): string => value.replace(HEX_PREFIX_RE, '').toLowerCase(),
  sameIdentity = (a: string, b: string): boolean => normalizeIdentity(a) === normalizeIdentity(b),
  toInt = (value: unknown): number => {
    if (typeof value === 'number') return value
    if (typeof value === 'string') return Number.parseInt(value, 10)
    return Number.NaN
  },
  escapeSqlString = (value: string): string => value.replaceAll("'", "''"),
  ...
  getCurrentIdentity = async (identityKey = 'default'): Promise<string> => (await getTokenFor(identityKey)).identity,
  querySingle = async <T>(sql: string): Promise<null | T> => {
    const rows = await querySQL<T>(sql),
      [first] = rows
    if (!first) return null
    return first
  },
  mapOrg = (row: Record<string, unknown>) => ({
    _id: toId(row.id),
  ...
  getMembership = async (orgId: string, identityKey = 'default') => {
    const id = toInt(orgId)
    if (!Number.isFinite(id)) return null
    const userId = await getCurrentIdentity(identityKey),
      row = await querySingle<Record<string, unknown>>(
  ...
  getMyOrgs = async (identityKey = 'default') => {
    const userId = await getCurrentIdentity(identityKey),
      owned = await querySQL<Record<string, unknown>>(`SELECT * FROM org WHERE user_id = '${escapeSqlString(userId)}'`),
  ...
  getPendingInvites = async (orgId: string) => {
    const id = toInt(orgId)
    if (!Number.isFinite(id)) return []
  ...
  mapProject = (row: Record<string, unknown>) => ({
    _id: toId(row.id),
  ...
  getProjectById = async (id: string, orgId: string) => {
    const parsedId = toInt(id),
      parsedOrgId = toInt(orgId)
  ...
  listProjects = async (orgId: string, numItems: number) => {
  ...
  getTaskById = async (id: string, orgId: string) => {
    const parsedId = toInt(id),
      parsedOrgId = toInt(orgId)
    if (!(Number.isFinite(parsedId) && Number.isFinite(parsedOrgId))) return null
    const row = await querySingle<Record<string, unknown>>(
  ...
  mapWiki = (row: Record<string, unknown>) => ({
    _id: toId(row.id),
    content: row.content,
  ...
  getWikiById = async (id: string, orgId: string) => {
    const parsedId = toInt(id),
      parsedOrgId = toInt(orgId)
    if (!(Number.isFinite(parsedId) && Number.isFinite(parsedOrgId))) return null
    const row = await querySingle<Record<string, unknown>>(
  ...
  listWikis = async (orgId: string, numItems: number) => {
    const parsedOrgId = toInt(orgId)
    if (!Number.isFinite(parsedOrgId)) return { continueCursor: null, isDone: true, page: [] }
    const rows = await querySQL<Record<string, unknown>>(
  ...
  createTestUser = async (email: string, name: string) => {
    const key = `user:${email}`,
      created = await getTokenFor(key),
      identity = created.identity.startsWith('0x') ? created.identity : `0x${created.identity}`
    identityKeyByUserId.set(created.identity, key)
  ...
  pickLatestInvite = (invites: Record<string, unknown>[]) => {
  ...
  pickLatestById = (rows: Record<string, unknown>[]) => {
    let latest: null | Record<string, unknown> = null,
      latestId = -1
    for (const row of rows) {
  ...
  addTestOrgMember = async (orgId: Id<'org'> | string, userId: Id<'users'> | string, isAdmin: boolean) => {
    const userIdString = String(userId),
      ...
      email = emailByUserId.get(userIdString) ?? `${userIdString}@test.local`
    ...
    const invites = await querySQL<Record<string, unknown>>(
      ...
      invite = pickLatestInvite(invites)
    ...
    if (isAdmin && !member.isAdmin) await callReducer('org_set_admin', { isAdmin: true, memberId: toInt(member.id) })

    return toId(member.id)
  },
  removeTestOrgMember = async (orgId: Id<'org'> | string, userId: Id<'users'> | string) => {
    const userIdString = String(userId),
      key = identityKeyByUserId.get(userIdString)
    if (key) {
  ...
  runMutation = async <T>(name: string, args: Record<string, unknown>): Promise<T> => {
    ...
    if (name === 'project:rm') {
      const projectId = toInt(args.id),
        ...
        tasks = await querySQL<Record<string, unknown>>(
    ...
    if (name === 'org:acceptInvite') {
      const token = String(args.token ?? ''),
        invite = await querySingle<Record<string, unknown>>(
    ...
    if (name === 'testauth:requestJoinAsUser') {
      const userId = String(args.userId ?? ''),
        ...
        key = identityKeyByUserId.get(userId)
    ...
    if (name === 'org:invite') {
      ...
      for (const invite of invites) if (invite.email === args.email) found = invite
  ...
  raw = {
    action: async <T>(name: string, args: Record<string, unknown>) =>
      expectError<T>(async () => runMutation<T>(name, args)),
    mutation: async <T>(name: string, args: Record<string, unknown>) =>
      expectError<T>(async () => runMutation<T>(name, args)),
    query: async <T>(name: string, args: Record<string, unknown>) => expectError<T>(async () => runQuery<T>(name, args))
  },
  tc = {
