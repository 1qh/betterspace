...
const TABLE_TYPES = new Set<TableType>(['cache', 'child', 'org', 'owned', 'singleton']),
  ...
  printAddHelp = () => {
    ...
    console.log(`  ${dim('$')} betterspace add todo --fields="title:string,done:boolean"`)
    console.log(
      `  ${dim('$')} betterspace add wiki --type=org --fields="title:string,content:string,status:enum(draft,published)"`
    )
