  ...
  printHealthReport = (convexDir: string, schemaFile: { content: string; path: string }) => {
    ...
    console.log(
      `  ${dim('Run')} betterspace check --schema ${dim('for schema preview')}\n` +
        `  ${dim('Run')} betterspace check --endpoints ${dim('for endpoint list')}\n` +
        `  ${dim('Run')} betterspace check --indexes ${dim('for index analysis')}\n` +
        `  ${dim('Run')} betterspace check --access ${dim('for access matrix')}\n`
    )
  },
  run = () => {
