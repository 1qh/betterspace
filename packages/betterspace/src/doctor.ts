...
const bold = (s: string) => `\u001B[1m${s}\u001B[0m`,
  ...
  checkEslintContent = (content?: string): CheckResult => {
    ...
    if (content.includes('betterspace/eslint'))
      return { details: ['betterspace/eslint plugin configured'], status: 'pass', title: 'ESLint Configuration' }
    return {
      details: ['eslint.config found but betterspace/eslint not imported'],
      status: 'warn',
  ...
  checkDeps = (pkg?: Record<string, unknown>): CheckResult => {
    ...
    const deps = (pkg.dependencies ?? {}) as Record<string, string>,
      ...
      all = { ...deps, ...devDeps },
      details: string[] = [],
      required = ['betterspace', 'spacetimedb', 'zod']
    let missing = 0
