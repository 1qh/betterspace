...
describe('betterspace-check --endpoints', () => {
...
describe('bundle verification', () => {
  ...
  test('betterspace/schema has no node:fs imports', async () => {
    const { readFileSync } = await import('node:fs'),
      { join } = await import('node:path'),
      content = readFileSync(join(import.meta.dir, '..', 'schema.ts'), 'utf8')
  ...
  test('betterspace/retry has no React or server imports', async () => {
    const { readFileSync } = await import('node:fs'),
      { join } = await import('node:path'),
...
describe('security ESLint rules', () => {
  ...
  test('recommended config includes all 4 new rules', () => {
    ...
    expect(ruleNames).toContain('betterspace/require-rate-limit')
    expect(ruleNames).toContain('betterspace/no-unprotected-mutation')
    expect(ruleNames).toContain('betterspace/no-unlimited-file-size')
    expect(ruleNames).toContain('betterspace/no-empty-search-config')
  })
  ...
  test('require-rate-limit is warn level in recommended', () => {
    expect(eslintRecommended.rules['betterspace/require-rate-limit']).toBe('warn')
  })

  test('no-empty-search-config is error level in recommended', () => {
    expect(eslintRecommended.rules['betterspace/no-empty-search-config']).toBe('error')
  })

  test('no-unprotected-mutation is warn level in recommended', () => {
    expect(eslintRecommended.rules['betterspace/no-unprotected-mutation']).toBe('warn')
  })

  test('no-unlimited-file-size is warn level in recommended', () => {
    expect(eslintRecommended.rules['betterspace/no-unlimited-file-size']).toBe('warn')
  })
})
...
describe('betterspace add command', () => {
...
describe('doctor', () => {
  ...
  test('checkEslintContent — with plugin', () => {
    expect(checkEslintContent("import { recommended } from 'betterspace/eslint'").status).toBe('pass')
  })
  ...
  test('checkDeps — all present', () => {
    expect(checkDeps({ dependencies: { convex: '1', betterspace: '2', zod: '3' } }).status).toBe('pass')
  })
  ...
  test('checkDeps — devDependencies count', () => {
    expect(checkDeps({ devDependencies: { convex: '1', betterspace: '2', zod: '3' } }).status).toBe('pass')
  })
