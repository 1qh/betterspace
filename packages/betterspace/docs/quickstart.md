...
```bash
bun add betterspace convex @convex-dev/auth zod
```
...
Follow the [Convex Auth guide](https://docs.convex.dev/auth) to configure authentication. betterspace requires `@convex-dev/auth` — every factory enforces auth by default.
...
```tsx
import { Form, Text, Toggle, Choose } from 'betterspace/components'
