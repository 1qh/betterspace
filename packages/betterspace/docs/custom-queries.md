...
You can also drop to raw Convex `action`/`mutation`/`query` when you don't need betterspace's auth context:
...
| Rate limiting | `rateLimit` option | Manual (`checkRateLimit` from `betterspace/server`) |
...
Need to call an external API?
  └─ Use raw Convex action (not a betterspace builder)
