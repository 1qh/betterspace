...
betterspace includes a Swift codegen CLI and 8 native apps (4 mobile + 4 desktop) that consume the same Convex backend as the web demos.
...
packages/betterspace/  Codegen CLI (codegen-swift.ts)
```
...
```bash
bunx betterspace codegen-swift \
  --schema convex/t.ts \
  --convex convex/ \
...
2. Re-run `bunx betterspace codegen-swift` with the same flags
...
```bash
bunx betterspace codegen-swift --schema convex/t.ts --convex convex/ --output Generated.swift
