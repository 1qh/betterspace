...
const require_safe_stable_stringify = __commonJS((exports, module) => {
...
const getOwnedRow = ({ ctxSender, id, operation, pkAccessor, table, tableName }) => {
...
const makeChildCrud = (spacetimedb, config) => {
  ...
  const createReducer = spacetimedb.reducer({ name: createName }, createParams, (ctx, args) => {
      const hookCtx = { db: ctx.db, sender: ctx.sender, timestamp: ctx.timestamp },
        ...
        argsRecord = args,
        parentId = argsRecord[foreignKeyName],
...
const $fromByteArray = fromByteArray
const lookup = []
const revLookup = []
...
function encodeChunk(uint8, start, end) {
  ...
  for (let i2 = start; i2 < end; i2 += 3) {
    tmp = ((uint8[i2] << 16) & 16_711_680) + ((uint8[i2 + 1] << 8) & 65_280) + (uint8[i2 + 2] & 255)
...
const import___ = __toESM(require_safe_stable_stringify(), 1)
const configure = import___.default.configure
const stringify = import___.default
...
function aborted(x, startIndex = 0) {
  ...
  for (let i2 = startIndex; i2 < x.issues.length; i2 += 1) {
...
function base64ToUint8Array(base64) {
  ...
  for (let i2 = 0; i2 < binaryString.length; i2 += 1) {
    bytes[i2] = binaryString.codePointAt(i2)
  }
  return bytes
}
function uint8ArrayToBase64(bytes) {
  ...
  for (let i2 = 0; i2 < bytes.length; i2 += 1) {
    binaryString += String.fromCodePoint(bytes[i2])
  }
...
function uint8ArrayToBase64url(bytes) {
  return uint8ArrayToBase64(bytes).replaceAll(/\+/g, '-').replaceAll(/\//g, '_').replaceAll(/[=]/g, '')
}
...
const $ZodNullable = /* @__PURE__ */ $constructor('$ZodNullable', (inst, def) => {
  ...
  defineLazy(inst._zod, 'values', () =>
    def.innerType._zod.values ? new Set([...def.innerType._zod.values, null]) : undefined
  )
  inst._zod.parse = (payload, ctx) => {
...
const WRAPPERS = new Set(['catch', 'default', 'nullable', 'optional', 'prefault', 'readonly'])
...
const err = (code2, opts) => {
  if (!opts) throw new SenderError(serializeError({ code: code2 }))
  if (typeof opts !== 'string') throw new SenderError(serializeError({ code: code2, ...opts }))
  const sep = opts.indexOf(':'),
    data =
      sep > 0
        ? { code: code2, debug: opts, op: opts.slice(sep + 1), table: opts.slice(0, sep) }
        : { code: code2, debug: opts }
  throw new SenderError(serializeError(data))
}
const time2 = timestamp => ({ updatedAt: timestamp ?? Date.now() })
const errValidation = (code2, zodError) => {
...
const checkRateLimit = async (db, opts) => {
  const { config: config2, key, table } = opts,
  ...
  if (existing.count >= config2.max) {
    const windowStart = existing.windowStart,
      retryAfter = config2.window - (now - windowStart)
...
const parseSenderMessage = message => {
  ...
  if (!(code2 in ERROR_MESSAGES)) return
...
const requireAdminRole = ({ operation, org, orgMemberTable, sender }) => {
...
const resolveAcceptedInvite = ({ inviteByTokenIndex, orgMemberTable, orgPk, sender, token }) => {
...
const completeInviteAcceptance = ({
  ...
  if (pendingRequest) requestPk.update({ ...pendingRequest, status: 'approved' })
...
const makeInviteReducers = (spacetimedb, config2) => {
  const inviteReducer = spacetimedb.reducer(
      ...
      (ctx, args) => {
        const orgTable = config2.orgTable(ctx.db),
          orgPk = config2.orgPk(orgTable),
          orgMemberTable = config2.orgMemberTable(ctx.db),
          orgInviteTable = config2.orgInviteTable(ctx.db),
    ...
    acceptInviteReducer = spacetimedb.reducer(
      ...
      (ctx, args) => {
        const orgTable = config2.orgTable(ctx.db),
          orgInviteTable = config2.orgInviteTable(ctx.db),
          orgJoinRequestTable = config2.orgJoinRequestTable(ctx.db)
        acceptInvite({
          ctx,
...
const requireAdminRole2 = ({ operation, org, orgMemberTable, sender }) => {
  const member = findOrgMember2(orgMemberTable, org.id, sender),
    role = getRole2(org, member, sender)
  if (!role) throw makeError('NOT_ORG_MEMBER', `org:${operation}`)
  if (role === 'member') throw makeError('FORBIDDEN', `org:${operation}`)
}
  ...
  const requestJoinReducer = spacetimedb.reducer(
      ...
      (ctx, args) => {
        const orgTable = config2.orgTable(ctx.db),
          ...
          orgMemberTable = config2.orgMemberTable(ctx.db),
          orgJoinRequestTable = config2.orgJoinRequestTable(ctx.db),
          joinByOrgStatusIndex = config2.orgJoinRequestByOrgStatusIndex(orgJoinRequestTable),
          org = orgPk.find(args.orgId)
    ...
    approveJoinReducer = spacetimedb.reducer(
      ...
      (ctx, args) => {
        const orgTable = config2.orgTable(ctx.db),
          ...
          orgMemberTable = config2.orgMemberTable(ctx.db),
          orgJoinRequestTable = config2.orgJoinRequestTable(ctx.db),
          orgJoinRequestPk = config2.orgJoinRequestPk(orgJoinRequestTable),
    ...
    cancelJoinReducer = spacetimedb.reducer(
      ...
      { requestId: config2.builders.requestId },
      (ctx, args) => {
        const orgJoinRequestTable = config2.orgJoinRequestTable(ctx.db),
          orgJoinRequestPk = config2.orgJoinRequestPk(orgJoinRequestTable),
          request = orgJoinRequestPk.find(args.requestId)
...
const requireRole = ({ minRole, operation, org, orgMemberTable, sender, tableName }) => {
  const member = findOrgMember3(orgMemberTable, org.id, sender),
    role = getRole3(org, member, sender)
...
const makeMemberReducers = (spacetimedb, config2) => {
  const setAdminReducer = spacetimedb.reducer(
      { name: 'org_set_admin' },
      { isAdmin: config2.builders.isAdmin, memberId: config2.builders.memberId },
      (ctx, args) => {
        const orgTable = config2.orgTable(ctx.db),
          orgPk = config2.orgPk(orgTable),
          orgMemberTable = config2.orgMemberTable(ctx.db),
          orgMemberPk = config2.orgMemberPk(orgMemberTable),
...
const requireRole2 = ({ minRole, operation, org, orgMemberTable, sender }) => {
  const member = findOrgMember4(orgMemberTable, org.id, sender),
    role = getRole4(org, member, sender)
  if (!role) throw makeError('NOT_ORG_MEMBER', `org:${operation}`)
...
const applyOrgUpdate = opts => {
  ...
  const nextRecord = { ...opts.org, updatedAt: opts.timestamp },
...
const makeOrg = (spacetimedb, config2) => {
  const orgFields = config2.fields,
    optionalOrgFields = makeOptionalFields2(orgFields),
    updateParams = {
      orgId: config2.builders.orgId
    },
  ...
  const createReducer = spacetimedb.reducer({ name: 'org_create' }, orgFields, (ctx, args) => {
      const orgTable = config2.orgTable(ctx.db),
        orgSlugIndex = config2.orgSlugIndex(orgTable),
        { slug } = args
    ...
    removeReducer = spacetimedb.reducer({ name: 'org_remove' }, { orgId: config2.builders.orgId }, (ctx, args) => {
      ...
      const joinByOrg = config2.orgJoinRequestByOrgIndex(orgJoinRequestTable),
        inviteByOrg = config2.orgInviteByOrgIndex(orgInviteTable),
        memberByOrg = config2.orgMemberByOrgIndex(orgMemberTable)
...
const requireMembership = ({ operation, orgId, orgMemberTable, sender, tableName }) => {
  const member = checkMembership(orgMemberTable, orgId, sender)
  if (!member) throw makeError('NOT_ORG_MEMBER', `${tableName}:${operation}`)
  return member
}
const requireCanMutate = ({ member, operation, row, sender, tableName }) => {
  if (member.isAdmin) return
  if (!identityEquals(row.userId, sender)) throw makeError('FORBIDDEN', `${tableName}:${operation}`)
}
const getOrgOwnedRow = ({ id, operation, orgMemberTable, pkAccessor, sender, table, tableName }) => {
  const pk = pkAccessor(table),
    row = pk.find(id)
  if (!row) throw makeError('NOT_FOUND', `${tableName}:${operation}`)
...
const makeOrgCrud = (spacetimedb, config2) => {
  const {
    ...
    optionalFields = makeOptionalFields(fields),
    optionalKeys2 = Object.keys(optionalFields)
  for (const key of optionalKeys2) {
    const field = optionalFields[key]
    if (field) updateParams[key] = field
  }
  if (expectedUpdatedAtField) updateParams.expectedUpdatedAt = expectedUpdatedAtField.optional()
  const createParams = { orgId: orgIdField },
    fieldKeys = Object.keys(fields)
  for (const key of fieldKeys) createParams[key] = fields[key]
  const createReducer = spacetimedb.reducer({ name: createName }, createParams, (ctx, args) => {
      const hookCtx = { db: ctx.db, sender: ctx.sender, timestamp: ctx.timestamp },
        table = tableAccessor(ctx.db),
...
const isAuthenticated = sender => {
  const senderLike = sender,
    raw = typeof senderLike.toHexString === 'function' ? senderLike.toHexString() : (senderLike.toString?.() ?? ''),
    normalized = raw.trim().toLowerCase().replace(ZERO_PREFIX_REGEX2, '')
  if (!normalized) return false
  for (const ch of normalized) if (ch !== '0') return true
  return false
}
...
const upsertPresence = ({ args, ctx, pk, table }) => {
...
const makePresence = (spacetimedb, config2) => {
  const { dataField, pk: pkAccessor, roomIdField, table: tableAccessor, tableName = 'presence' } = config2,
...
const childTable = (s, indexField, indexName) =>
  asNever(
    tableDef('child', {
      ...zodShapeToFields(s.shape),
      updatedAt: 'number'
...
const checkSchema = schemas2 => {
  const res = []
  for (const [table, schema] of Object.entries(schemas2)) scanSchema(schema, table, res)
  if (res.length > 0) {
    for (const f of res)
...
const makeSingletonCrud = (spacetimedb, config2) => {
  const { fields, options, table: tableAccessor, tableName } = config2,
    hooks = options?.hooks,
    fieldNames = Object.keys(fields),
...
const mergeSingletonBeforeUpdate = (table, globalHooks, localHooks) => {
  ...
  return (ctx, { patch: initialPatch, prev }) => {
    ...
    if (globalHooks?.beforeUpdate)
      patch = requireSync(
        globalHooks.beforeUpdate(toGlobalCtx(table, ctx), { patch, prev }),
...
const setup = (spacetimedb, config2 = {}) => {
  const middlewareHooks = config2.middleware?.length ? composeMiddleware(...config2.middleware) : undefined,
    globalHooks = mergeGlobalHooks(config2.hooks, middlewareHooks),
    accumulatedExports = {},
    crud = factoryConfig => {
...
const TimeDuration = class _TimeDuration {
...
const Timestamp = class _Timestamp {
...
const Uuid = class _Uuid {
  ...
  static fromCounterV7(counter, now, randomBytes) {
    ...
    counter.value = (counterVal + 1) & 2_147_483_647
  ...
  static parse(s) {
    ...
    for (let i2 = 0; i2 < 32; i2 += 2) {
      v = (v << 8n) | BigInt(Number.parseInt(hex.slice(i2, i2 + 2), 16))
    }
    return new _Uuid(v)
  }
  toString() {
...
function uint8ArrayToHexString(array2) {
  return Array.prototype.map.call(array2.toReversed(), x => ('00' + x.toString(16)).slice(-2)).join('')
}
...
function hexStringToUint8Array(str) {
  ...
  const data = Uint8Array.from(matches.map(byte => Number.parseInt(byte, 16)))
...
const Identity = class _Identity {
  __identity__
  constructor(data) {
...
const isFixedSizeProduct = ty => ty.elements.every(({ algebraicType }) => fixedSizePrimitives.has(algebraicType.tag))
const productSize = ty => ty.elements.reduce((acc, { algebraicType }) => acc + primitiveSizes[algebraicType.tag], 0)
const primitiveJSName = {
  Bool: 'Uint8',
  I8: 'Int8',
  U8: 'Uint8',
  I16: 'Int16',
...
const unitDeserializer = () => ({})
const getElementInitializer = element => {
  let init
...
${ty.elements
  .map(({ name, algebraicType: { tag } }) =>
    tag in primitiveJSName
      ? `view.set${primitiveJSName[tag]}(writer.offset, value.${name}, ${primitiveSizes[tag] > 1 ? 'true' : ''});
...
` +
      ty.elements
        .map(element => `this.${element.name}(writer, value.${element.name});`)
        .join(`
`)
...
const result = { ${ty.elements.map(getElementInitializer).join(', ')} };
${ty.elements
  .map(({ name }) => `result.${name} = this.${name}(reader);`)
  .join(`
`)}
return result;`
    ).bind(deserializers)
...
const SumType = {
  makeSerializer(ty, typespace) {
    ...
    if (ty.variants.length == 2 && ty.variants[0].name === 'ok' && ty.variants[1].name === 'err') {
...
${ty.variants
  .map(
    ({ name }, i2) => `  case ${JSON.stringify(name)}:
    writer.writeByte(${i2});
...
${ty.variants
  .map(({ name }, i2) => `case ${i2}: return { tag: ${JSON.stringify(name)}, value: this.${name}(reader) };`)
  .join(`
`)} }`
    ).bind(deserializers)
    DESERIALIZERS.set(ty, deserializer)
...
const ConnectionId = class _ConnectionId {
  __connection_id__
  constructor(data) {
  ...
  static getAlgebraicType() {
    return AlgebraicType.Product({
      elements: [{ name: '__connection_id__', algebraicType: AlgebraicType.U128 }]
    })
  }
  isZero() {
...
const SenderError2 = class extends Error {
  constructor(message) {
    super(message)
  }
  get name() {
...
const Interval = micros => ({
  tag: 'Interval',
  value: new TimeDuration(micros)
})
const Time = microsSinceUnixEpoch => ({
  tag: 'Time',
  value: new Timestamp(microsSinceUnixEpoch)
})
const schedule_at_default = ScheduleAt
const Option = {
...
const isRowTypedQuery = val => !!val && typeof val === 'object' && QueryBrand in val
function toSql(q) {
  return q.toSql()
}
const SemijoinImpl = class _SemijoinImpl {
...
const FromBuilder = class _FromBuilder {
...
const BooleanExpr = class _BooleanExpr {
  constructor(data) {
    this.data = data
  }
  and(other) {
...
const U8Builder = class extends TypeBuilder {
  constructor() {
...
const U16Builder = class extends TypeBuilder {
  constructor() {
...
const U32Builder = class extends TypeBuilder {
  constructor() {
...
const U64Builder = class extends TypeBuilder {
  constructor() {
...
const U128Builder = class extends TypeBuilder {
  constructor() {
...
const U256Builder = class extends TypeBuilder {
  constructor() {
...
const I8Builder = class extends TypeBuilder {
  constructor() {
...
const I16Builder = class extends TypeBuilder {
  constructor() {
...
const I32Builder = class extends TypeBuilder {
  constructor() {
...
const I64Builder = class extends TypeBuilder {
  constructor() {
...
const I128Builder = class extends TypeBuilder {
  constructor() {
...
const I256Builder = class extends TypeBuilder {
  constructor() {
...
const F32Builder = class extends TypeBuilder {
  constructor() {
...
const F64Builder = class extends TypeBuilder {
  constructor() {
...
const BoolBuilder = class extends TypeBuilder {
  constructor() {
...
const StringBuilder = class extends TypeBuilder {
  constructor() {
...
const ArrayBuilder = class extends TypeBuilder {
  element
  constructor(element) {
...
const ByteArrayBuilder = class extends TypeBuilder {
  constructor() {
...
const OptionBuilder = class extends TypeBuilder {
  value
  constructor(value) {
...
const ProductBuilder = class extends TypeBuilder {
  typeName
  elements
...
const ResultBuilder = class extends TypeBuilder {
  ok
  err
...
const UnitBuilder = class extends TypeBuilder {
  constructor() {
    super({ tag: 'Product', value: { elements: [] } })
  }
}
const RowBuilder = class extends TypeBuilder {
  row
  typeName
...
const SumBuilderImpl = class extends TypeBuilder {
  variants
  typeName
...
const SumBuilder = SumBuilderImpl
const SimpleSumBuilderImpl = class extends SumBuilderImpl {
  index(algorithm = 'btree') {
...
const ScheduleAtBuilder = class extends TypeBuilder {
  constructor() {
...
const IdentityBuilder = class extends TypeBuilder {
  constructor() {
...
const ConnectionIdBuilder = class extends TypeBuilder {
  constructor() {
...
const TimestampBuilder = class extends TypeBuilder {
  constructor() {
...
const TimeDurationBuilder = class extends TypeBuilder {
  constructor() {
...
const UuidBuilder = class extends TypeBuilder {
  constructor() {
...
const U8ColumnBuilder = class _U8ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const U16ColumnBuilder = class _U16ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const U32ColumnBuilder = class _U32ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const U64ColumnBuilder = class _U64ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const U128ColumnBuilder = class _U128ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const U256ColumnBuilder = class _U256ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const I8ColumnBuilder = class _I8ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const I16ColumnBuilder = class _I16ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const I32ColumnBuilder = class _I32ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const I64ColumnBuilder = class _I64ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const I128ColumnBuilder = class _I128ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const I256ColumnBuilder = class _I256ColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const F32ColumnBuilder = class _F32ColumnBuilder extends ColumnBuilder {
  default(value) {
...
const F64ColumnBuilder = class _F64ColumnBuilder extends ColumnBuilder {
  default(value) {
...
const BoolColumnBuilder = class _BoolColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const StringColumnBuilder = class _StringColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const ArrayColumnBuilder = class _ArrayColumnBuilder extends ColumnBuilder {
  default(value) {
...
const ByteArrayColumnBuilder = class _ByteArrayColumnBuilder extends ColumnBuilder {
...
const OptionColumnBuilder = class _OptionColumnBuilder extends ColumnBuilder {
  default(value) {
...
const ResultColumnBuilder = class _ResultColumnBuilder extends ColumnBuilder {
  constructor(typeBuilder, metadata) {
...
const ProductColumnBuilder = class _ProductColumnBuilder extends ColumnBuilder {
  default(value) {
...
const SumColumnBuilder = class _SumColumnBuilder extends ColumnBuilder {
  default(value) {
...
const SimpleSumColumnBuilder = class _SimpleSumColumnBuilder extends SumColumnBuilder {
  index(algorithm = 'btree') {
...
const ScheduleAtColumnBuilder = class _ScheduleAtColumnBuilder extends ColumnBuilder {
  default(value) {
...
const IdentityColumnBuilder = class _IdentityColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const ConnectionIdColumnBuilder = class _ConnectionIdColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const TimestampColumnBuilder = class _TimestampColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const TimeDurationColumnBuilder = class _TimeDurationColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const UuidColumnBuilder = class _UuidColumnBuilder extends ColumnBuilder {
  index(algorithm = 'btree') {
...
const RefBuilder = class extends TypeBuilder {
  ref
  __spacetimeType
  constructor(ref) {
...
const enumImpl = (nameOrObj, maybeObj) => {
  let obj = nameOrObj
  let name
...
const t = {
  ...
  scheduleAt: () => new ScheduleAtBuilder(),
  option(value) {
  ...
  connectionId: () => new ConnectionIdBuilder(),
  timestamp: () => new TimestampBuilder(),
  timeDuration: () => new TimeDurationBuilder(),
  uuid: () => new UuidBuilder(),
  byteArray: () => new ByteArrayBuilder()
}
const BsatnRowList = t.object('BsatnRowList', {
  get sizeHint() {
    return RowSizeHint
  },
  rowsData: t.byteArray()
})
const CallProcedure = t.object('CallProcedure', {
  requestId: t.u32(),
  flags: t.u8(),
  procedure: t.string(),
  args: t.byteArray()
})
const CallReducer = t.object('CallReducer', {
  requestId: t.u32(),
  flags: t.u8(),
  reducer: t.string(),
  args: t.byteArray()
})
const ClientMessage = t.enum('ClientMessage', {
  get Subscribe() {
    return Subscribe
  },
  get Unsubscribe() {
...
const EventTableRows = t.object('EventTableRows', {
  get events() {
    return BsatnRowList
  }
})
const InitialConnection = t.object('InitialConnection', {
  identity: t.identity(),
  connectionId: t.connectionId(),
  token: t.string()
})
const OneOffQuery = t.object('OneOffQuery', {
  requestId: t.u32(),
  queryString: t.string()
})
const OneOffQueryResult = t.object('OneOffQueryResult', {
  requestId: t.u32(),
  get result() {
    return t.result(QueryRows, t.string())
  }
})
const PersistentTableRows = t.object('PersistentTableRows', {
  get inserts() {
    return BsatnRowList
  },
  get deletes() {
...
const ProcedureResult = t.object('ProcedureResult', {
  get status() {
...
const ProcedureStatus = t.enum('ProcedureStatus', {
  Returned: t.byteArray(),
  InternalError: t.string()
})
const QueryRows = t.object('QueryRows', {
  get tables() {
    return t.array(SingleTableRows)
  }
})
const QuerySetId = t.object('QuerySetId', {
  id: t.u32()
})
const QuerySetUpdate = t.object('QuerySetUpdate', {
  get querySetId() {
    return QuerySetId
  },
  get tables() {
...
const ReducerOk = t.object('ReducerOk', {
  retValue: t.byteArray(),
  get transactionUpdate() {
    return TransactionUpdate
  }
})
const ReducerOutcome = t.enum('ReducerOutcome', {
  get Ok() {
    return ReducerOk
  },
  OkEmpty: t.unit(),
...
const ReducerResult = t.object('ReducerResult', {
  requestId: t.u32(),
  timestamp: t.timestamp(),
  get result() {
...
const RowSizeHint = t.enum('RowSizeHint', {
  FixedSize: t.u16(),
  RowOffsets: t.array(t.u64())
})
const ServerMessage = t.enum('ServerMessage', {
...
const SingleTableRows = t.object('SingleTableRows', {
  table: t.string(),
  get rows() {
    return BsatnRowList
  }
})
const Subscribe = t.object('Subscribe', {
  requestId: t.u32(),
  get querySetId() {
    return QuerySetId
  },
...
const SubscribeApplied = t.object('SubscribeApplied', {
  requestId: t.u32(),
  get querySetId() {
    return QuerySetId
  },
...
const SubscriptionError = t.object('SubscriptionError', {
...
const TableUpdate = t.object('TableUpdate', {
  tableName: t.string(),
  get rows() {
    return t.array(TableUpdateRows)
  }
})
const TableUpdateRows = t.enum('TableUpdateRows', {
  get PersistentTable() {
...
const TransactionUpdate = t.object('TransactionUpdate', {
  get querySets() {
    return t.array(QuerySetUpdate)
  }
})
const Unsubscribe = t.object('Unsubscribe', {
  requestId: t.u32(),
  get querySetId() {
    return QuerySetId
  },
...
const UnsubscribeApplied = t.object('UnsubscribeApplied', {
  requestId: t.u32(),
  get querySetId() {
    return QuerySetId
  },
...
const UnsubscribeFlags = t.enum('UnsubscribeFlags', {
  Default: t.unit(),
...
const shouldLog = level => LogLevelRank[level] <= LogLevelRank[globalLogLevel]
const resolveLazy = v => (typeof v === 'function' ? v() : v)
const toHex2 = bytes =>
  Array.from(bytes)
    .map(b => b.toString(16).padStart(2, '0'))
    .join('')
...
const SENSITIVE_KEYS = /* @__PURE__ */ new Set(['token', 'authToken', 'authorization', 'accessToken', 'refreshToken'])
const stringify2 = value =>
  stringify(value, (key, current) => {
    ...
    if (
      ...
      typeof current.__connection_id__ === 'bigint'
    ) {
...
const stdbLogger = (level, message, ...args) => {
  if (!shouldLog(level)) {
    return
  }
...
const scalarCompare = (x, y) => {
  if (x === y) return 0
  return x < y ? -1 : 1
}
const TableCacheImpl = class {
  rows
  tableDef
  emitter
  constructor(tableDef2) {
  ...
  #makeReadonlyIndex(tableDef2, idx2) {
    ...
    const matchRange = (row, rangeArg) => {
      ...
      for (let i2 = 0; i2 < prefixLen; i2++) {
        if (!deepEqual(key[i2], arr[i2])) return false
      }
...
const SemanticVersion = class _SemanticVersion {
  major
  minor
  patch
  preRelease
...
const _MINIMUM_CLI_VERSION = new SemanticVersion(1, 4, 0)
...
const WebsocketDecompressAdapter = class _WebsocketDecompressAdapter {
  onclose
  onopen
  onmessage
  onerror
  ...
  static async createWebSocketFn({ url, nameOrAddress, wsProtocol, authToken, compression, lightMode, confirmedReads }) {
...
const DbConnectionBuilder = class {
...
const INTERNAL_REMOTE_MODULE = Symbol('INTERNAL_REMOTE_MODULE')
const SubscriptionBuilderImpl = class {
  constructor(db) {
    this.db = db
  }
  #onApplied = undefined
...
const SubscriptionHandleImpl = class {
...
const DbConnectionImpl = class {
    ...
    for (const procedure of def.procedures) {
      ...
      out[key] = params => {
        ...
        return this.callProcedure(procedureName, argsBuffer).then(returnBuf =>
          deserializeReturn(new BinaryReader(returnBuf))
        )
      }
    }
    return out
  ...
  #handleOnMessage(wsMessage) {
    this.#messageQueue = this.#messageQueue.then(() => this.#processMessage(wsMessage.data))
  }
  ...
  callProcedureWithParams(procedureName, _paramsType, params, _returnType) {
    ...
    serializeArgs(writer, params)
    const argsBuffer = writer.getBuffer()
    return this.callProcedure(procedureName, argsBuffer).then(returnBuf => deserializeReturn(new BinaryReader(returnBuf)))
  }
  disconnect() {
    this.wsPromise.then(wsResolved => {
      if (wsResolved) {
...
const AlgebraicType2 = t.enum('AlgebraicType', {
  Ref: t.u32(),
  get Sum() {
    return SumType2
  },
...
const CaseConversionPolicy = t.enum('CaseConversionPolicy', {
  None: t.unit(),
  SnakeCase: t.unit()
})
const ExplicitNameEntry = t.enum('ExplicitNameEntry', {
  get Table() {
    return NameMapping
  },
  get Function() {
...
const ExplicitNames = t.object('ExplicitNames', {
  get entries() {
    return t.array(ExplicitNameEntry)
  }
})
const FunctionVisibility = t.enum('FunctionVisibility', {
  Private: t.unit(),
  ClientCallable: t.unit()
})
const HttpHeaderPair = t.object('HttpHeaderPair', {
  name: t.string(),
  value: t.byteArray()
})
const HttpHeaders = t.object('HttpHeaders', {
  get entries() {
    return t.array(HttpHeaderPair)
  }
})
const HttpMethod = t.enum('HttpMethod', {
  Get: t.unit(),
  Head: t.unit(),
  Post: t.unit(),
  Put: t.unit(),
...
const HttpVersion = t.enum('HttpVersion', {
  Http09: t.unit(),
  Http10: t.unit(),
  Http11: t.unit(),
  Http2: t.unit(),
...
const IndexType = t.enum('IndexType', {
  BTree: t.unit(),
  Hash: t.unit()
})
const Lifecycle = t.enum('Lifecycle', {
  Init: t.unit(),
  OnConnect: t.unit(),
  OnDisconnect: t.unit()
})
const MiscModuleExport = t.enum('MiscModuleExport', {
  get TypeAlias() {
    return TypeAlias
  }
})
const NameMapping = t.object('NameMapping', {
  sourceName: t.string(),
  canonicalName: t.string()
})
const ProductType2 = t.object('ProductType', {
  get elements() {
    return t.array(ProductTypeElement)
  }
})
const ProductTypeElement = t.object('ProductTypeElement', {
  name: t.option(t.string()),
  get algebraicType() {
    return AlgebraicType2
  }
})
const RawColumnDefV8 = t.object('RawColumnDefV8', {
  colName: t.string(),
  get colType() {
    return AlgebraicType2
  }
})
const RawColumnDefaultValueV10 = t.object('RawColumnDefaultValueV10', {
  colId: t.u16(),
  value: t.byteArray()
})
const RawColumnDefaultValueV9 = t.object('RawColumnDefaultValueV9', {
  table: t.string(),
  colId: t.u16(),
  value: t.byteArray()
})
const RawConstraintDataV9 = t.enum('RawConstraintDataV9', {
  get Unique() {
    return RawUniqueConstraintDataV9
  }
})
const RawConstraintDefV10 = t.object('RawConstraintDefV10', {
  sourceName: t.option(t.string()),
  get data() {
    return RawConstraintDataV9
  }
})
const RawConstraintDefV8 = t.object('RawConstraintDefV8', {
  constraintName: t.string(),
  constraints: t.u8(),
  columns: t.array(t.u16())
})
const RawConstraintDefV9 = t.object('RawConstraintDefV9', {
  name: t.option(t.string()),
  get data() {
    return RawConstraintDataV9
  }
})
const RawIndexAlgorithm = t.enum('RawIndexAlgorithm', {
  BTree: t.array(t.u16()),
  Hash: t.array(t.u16()),
  Direct: t.u16()
})
const RawIndexDefV10 = t.object('RawIndexDefV10', {
  sourceName: t.option(t.string()),
...
const RawIndexDefV8 = t.object('RawIndexDefV8', {
  indexName: t.string(),
  isUnique: t.bool(),
  get indexType() {
    return IndexType
...
const RawIndexDefV9 = t.object('RawIndexDefV9', {
  name: t.option(t.string()),
  accessorName: t.option(t.string()),
  get algorithm() {
    return RawIndexAlgorithm
...
const RawLifeCycleReducerDefV10 = t.object('RawLifeCycleReducerDefV10', {
  get lifecycleSpec() {
    return Lifecycle
  },
  functionName: t.string()
})
const RawMiscModuleExportV9 = t.enum('RawMiscModuleExportV9', {
...
const RawModuleDefV10 = t.object('RawModuleDefV10', {
  get sections() {
    return t.array(RawModuleDefV10Section)
  }
})
const RawModuleDefV10Section = t.enum('RawModuleDefV10Section', {
  get Typespace() {
    return Typespace
  },
  get Types() {
...
const RawModuleDefV8 = t.object('RawModuleDefV8', {
  get typespace() {
    return Typespace
  },
  get tables() {
...
const RawModuleDefV9 = t.object('RawModuleDefV9', {
  get typespace() {
    return Typespace
  },
  get tables() {
...
const RawProcedureDefV10 = t.object('RawProcedureDefV10', {
  sourceName: t.string(),
  get params() {
    return ProductType2
  },
...
const RawProcedureDefV9 = t.object('RawProcedureDefV9', {
  name: t.string(),
  get params() {
    return ProductType2
  },
...
const RawReducerDefV10 = t.object('RawReducerDefV10', {
  sourceName: t.string(),
  get params() {
    return ProductType2
  },
...
const RawReducerDefV9 = t.object('RawReducerDefV9', {
  name: t.string(),
  get params() {
    return ProductType2
  },
...
const RawRowLevelSecurityDefV9 = t.object('RawRowLevelSecurityDefV9', {
  sql: t.string()
})
const RawScheduleDefV10 = t.object('RawScheduleDefV10', {
  sourceName: t.option(t.string()),
  tableName: t.string(),
  scheduleAtCol: t.u16(),
  functionName: t.string()
})
const RawScheduleDefV9 = t.object('RawScheduleDefV9', {
  name: t.option(t.string()),
  reducerName: t.string(),
  scheduledAtColumn: t.u16()
})
const RawScopedTypeNameV10 = t.object('RawScopedTypeNameV10', {
  scope: t.array(t.string()),
  sourceName: t.string()
})
const RawScopedTypeNameV9 = t.object('RawScopedTypeNameV9', {
  scope: t.array(t.string()),
  name: t.string()
})
const RawSequenceDefV10 = t.object('RawSequenceDefV10', {
  sourceName: t.option(t.string()),
  column: t.u16(),
...
const RawSequenceDefV8 = t.object('RawSequenceDefV8', {
...
const RawSequenceDefV9 = t.object('RawSequenceDefV9', {
  name: t.option(t.string()),
  column: t.u16(),
  start: t.option(t.i128()),
  minValue: t.option(t.i128()),
...
const RawTableDefV10 = t.object('RawTableDefV10', {
  sourceName: t.string(),
...
const RawTableDefV8 = t.object('RawTableDefV8', {
  tableName: t.string(),
  get columns() {
...
const RawTableDefV9 = t.object('RawTableDefV9', {
  name: t.string(),
  productTypeRef: t.u32(),
  primaryKey: t.array(t.u16()),
  get indexes() {
...
const RawTypeDefV10 = t.object('RawTypeDefV10', {
  get sourceName() {
...
const RawTypeDefV9 = t.object('RawTypeDefV9', {
  get name() {
...
const RawUniqueConstraintDataV9 = t.object('RawUniqueConstraintDataV9', {
  columns: t.array(t.u16())
})
const RawViewDefV10 = t.object('RawViewDefV10', {
  sourceName: t.string(),
  index: t.u32(),
  isPublic: t.bool(),
  isAnonymous: t.bool(),
...
const RawViewDefV9 = t.object('RawViewDefV9', {
  name: t.string(),
  index: t.u32(),
  isPublic: t.bool(),
  isAnonymous: t.bool(),
...
const ReducerDef = t.object('ReducerDef', {
  name: t.string(),
  get args() {
    return t.array(ProductTypeElement)
  }
})
const SumType2 = t.object('SumType', {
  get variants() {
    return t.array(SumTypeVariant)
  }
})
const SumTypeVariant = t.object('SumTypeVariant', {
  name: t.option(t.string()),
  get algebraicType() {
    return AlgebraicType2
  }
})
const TableAccess = t.enum('TableAccess', {
  Public: t.unit(),
  Private: t.unit()
})
const TableDesc = t.object('TableDesc', {
  get schema() {
    return RawTableDefV8
  },
  data: t.u32()
})
const TableType = t.enum('TableType', {
  System: t.unit(),
  User: t.unit()
})
const TypeAlias = t.object('TypeAlias', {
  name: t.string(),
  ty: t.u32()
})
const Typespace = t.object('Typespace', {
  get types() {
...
const getSqlFields = schema => {
  ...
  const schemaRecord = schema,
    ...
    productElements =
      schemaRecord.Product && typeof schemaRecord.Product === 'object' ? schemaRecord.Product.elements : undefined,
    fields = [],
    elementsSource = Array.isArray(directElements) ? directElements : Array.isArray(productElements) ? productElements : []
...
const createTestContext = async options => {
  ...
  const pendingUsers = []
  for (let i2 = 1; i2 < userCount; i2 += 1) pendingUsers.push(createConnectedUser(ctx))
...
const discoverModules = async options => {
  const httpUrl = options?.httpUrl ?? DEFAULT_HTTP_URL2,
    moduleName = options?.moduleName ?? DEFAULT_MODULE_NAME2,
